	...
	<c:choose>
		<c:when test="${inputCell.field.inputType == 'Text'}">
			<input type="text"
				...
				<c:out value='${param.modelObject}'></c:out>.</c:if>
			<c:out value='${inputCell.field.name}'></c:out>"
				...
				<c:if test="${!empty inputCell.field.minLength}">minlength="<c:out
					value='${inputCell.field.minLength}'></c:out>"</c:if>
				<c:if test="${!empty inputCell.field.maxLength}">maxlength="<c:out
					value='${inputCell.field.maxLength}'></c:out>"</c:if>
				<c:if test="${!empty inputCell.field.inputPattern}">pattern="<c:out
					value='${inputCell.field.inputPattern}'></c:out>"</c:if>
				<c:if test="${!empty inputCell.field.patternMismatchMessage}">title="<c:out
					value='${inputCell.field.patternMismatchMessage}'></c:out>"</c:if>
